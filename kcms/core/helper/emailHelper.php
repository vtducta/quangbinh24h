<?php                
class emailHelper extends Helper implements IEmail
{
    /**
    * From email address 
    *
    ...
    */
    public function __construct($opts=array())
    {
        ...
        $cfg = Application::$config['mail']; 
        
        if(!count($cfg)) system_error("Config invalid, Please Check struct and data of mail.conf ");

        $cfg_smtp_host = isset($cfg['smtp_host']) ? $cfg['smtp_host'] : 'localhost';
        $cfg_smtp_port = isset($cfg['smtp_port']) ? $cfg['smtp_port'] : '25';
        $cfg_smtp_user = isset($cfg['smtp_user']) ? $cfg['smtp_user'] : ''; 
        $cfg_smtp_pass = isset($cfg['smtp_pass']) ? $cfg['smtp_pass'] : '';               
                
        $cfg_mail_method = isset($cfg['method']) ? $cfg['method'] : 'smtp';
        $cfg_html_email = isset($cfg['html']) ? $cfg['html'] : 0;
        $cfg_charset = isset($cfg['charset']) ? $cfg['charset'] : 'utf-8';
        
        $this->smtp_host        = !empty($opts['smtp_host']) ? $opts['smtp_host'] : $cfg_smtp_host;
        $this->smtp_port        = !empty($opts['smtp_port']) ? intval($opts['smtp_port']) : $cfg_smtp_port;
        $this->smtp_user        = !empty($opts['smtp_user']) ? $opts['smtp_user'] : $cfg_smtp_user;
        $this->smtp_pass        = !empty($opts['smtp_pass']) ? $opts['smtp_pass'] : $cfg_smtp_pass;
        
        
        $this->mail_method        = ( $opts['method'] AND in_array( strtolower($opts['method']), array( 'smtp', 'mail' ) ) )  ? strtolower($opts['method']) : $cfg_mail_method;
        $this->html_email        = !empty($opts['html']) ? $opts['html'] : $cfg_html_email;
        $this->char_set            = !empty($opts['charset']) ? $opts['charset'] : $cfg_charset;
    ...
    */
    protected function _buildHeaders()
    {
        ...
        $this->mail_headers['X-Mailer']                = "KCMS Mailer";
