                    ...
                    <div id="list_news">
                            ...
                            <table class="responsive table table-bordered" id="checkAll">
                                ...
                                <tbody>   
                                    ...
                                    <tr {if $smarty.foreach.foo.index %2!=0} class="second" {/if} >
                                        ...
                                        <td width="85"><a href="javascript:void(0)" class="list-post-thumb"><img src="{$template_helper->get_thumb_image($val.images,80,60)}"/></a></td>
                                        <td class="alignleft"><span class="lastedit-pe"></span><a href="javascript:void(0)"><strong>{$val.title|stripslashes}</strong></a>
                                            <p class="listing-lead">{$val.intro_text}</p>
                                            <div class="quk-edit">  
                                                <a href="{$link_helper->xemnhanh($val.meta_slug,$val.id)}" target="_blank">Xem nhanh</a> |
                                                <a href="{$link_helper->link_edit($data.edit_action,$val.id)}" target="_blank">Sửa</a>
                                            </div>
                                        </td>                                                
                                        <td>{$val.category}</td>                                                                                                
                        ...
                        <div class="margin10 clearfix">    
                            ...
                            <div class="pagination right">
                                <ul>
                                    <li><a href="{$link_helper->link_admin("MyPost",1)}"><span class="icon12 minia-icon-arrow-left-3"></span></a></li>
                                    {foreach from=$data.paging.page item=val key=k} 
                                    <li {if $val eq $data.paging.current} class="active"{/if}>
                                    <a href="{$link_helper->link_admin("MyPost",$val)}"><span>{$val}</span></a>
                                    </li>                                                    
                                    {/foreach}                                               
                                    <li><a href="{$link_helper->link_admin("MyPost",$data.paging.total)}"> <span class="icon12 minia-icon-arrow-right-3"></span></a></li>
                                </ul>
                            </div>                                        
                            {/if}                           
                        </div>
