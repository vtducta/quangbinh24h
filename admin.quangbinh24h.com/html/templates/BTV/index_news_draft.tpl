                                        ...
                                        <tbody>  
                                            {foreach from=$data.list_news item=val key=key name=foo}
                                                 <tr {if $smarty.foreach.foo.index %2!=0} class="second" {/if} >
                                                    ...
                                                    <td width="85"><a href="{$link_helper->link_edit("BtvEditNews",$val.id)}" class="list-post-thumb"><img src="{$template_helper->get_thumb_image($val.images,80,60)}"/></a></td>
                                                    <td class="alignleft"><span class="lastedit-pe"></span><a href="{$link_helper->link_edit("BtvEditNews",$val.id)}"><strong>{$val.title}</strong></a>
                                                        <p class="listing-lead">{$val.intro_text}</p>
                                                          <div class="quk-edit">                                                          
                                                            <a href="{$link_helper->link_edit("BtvEditNews",$val.id)}">Sửa</a>            
                                                            |
                                                            <a href="{$link_helper->link_deleted("publicnewsdraft",$val.id)}">Gửi bài duyệt</a>                   
                                                        </div>
                                                    </td>                                                
                                                    <td>{$val.category}</td>   
