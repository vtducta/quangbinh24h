<?php  
    class BtvNewsDraft extends Module implements IModule {  
        /**
        * tm groups cat model
        * 
        ...
        */
        ...
        public function run() {  
            ...
            $total= $this->news_articles_view_join_category_model->get_total_news_by_category("a.status=2 AND a.creat_by ={$user_id} AND a.public=3 AND a.timer =0","",0,"");           
            $total_page = ceil($total/$num);
            $paging     =   Application::paging($current_page,$total_page,$page_show);  
            $offset     =   ($current_page-1)*$num;
            $limit      =   $num;
            $list_news = $this->news_articles_view_join_category_model->admin_get_news_by_category_sql("a.status=2 AND  a.creat_by ={$user_id} AND a.public=3 AND a.timer =0","a.create_date_int DESC,a.id DESC",$offset,$limit);           
            ...
            $this->template->display("BTV/index_news_draft.tpl");            
        }
