<?php  
  class ReporterOff extends Module implements IModule {  
      /**
      * tm groups cat model
      * 
      ...
      */
      ...
      public function run() {  
           ...
           $total= $this->news_articles_model->get_total_by_sql("a.status=-1 AND a.creat_by ={$user_id}","",0,"");           
           $total_page = ceil($total/$num);
           $paging     =   Application::paging($current_page,$total_page,$page_show);  
           $offset     =   ($current_page-1)*$num;
           $limit      =   $num;
           $list_news = $this->news_articles_model->admin_get_news_by_mysql("status=-1 AND creat_by ={$user_id}","create_date_int DESC,id DESC",$offset,$limit);           
           ...
           $this->template->display("reporter/index_return.tpl");            
      }
