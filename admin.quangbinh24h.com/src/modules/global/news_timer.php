<?php
    class NewsTimer extends Module implements IModule {  
        /**
        ...
        */
        ...
        public function run() {  
            ...
            $total= $this->news_articles_join_category_view_model->get_total_news_by_category_sql("a.status=3 AND a.public=1 AND a.timer >0 ","",0,"");                      
            $total_page = ceil($total/$num);
            $paging     =   Application::paging($current_page,$total_page,$page_show);  
            $offset     =   ($current_page-1)*$num;
            $limit      =   $num;                                                                                  
            $list_news = $this->news_articles_join_category_view_model->admin_get_news_by_category_sql("a.status=3 AND a.public=1 AND a.timer >0 ","a.timer ASC,id DESC",$offset,$limit);               
            ...
            $this->template->display("reporter/index_timer.tpl");                                
        }
