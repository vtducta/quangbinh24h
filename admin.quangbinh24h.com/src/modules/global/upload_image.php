...
class UploadImage extends Module implements IModule
{  
    protected $options;
    ...
    function __construct($options = null, $initialize = true, $error_messages = null) {
          ...
          */          
          ...
          $this->options = array(
            'script_url' => 'http://media.quangbinh24h.com/',
            'upload_dir' => '/home/app/media.quangbinh24h.com/public_html/',            
            'upload_url' => 'http://media.quangbinh24h.com/',
            'user_dirs' => true,
            'mkdir_mode' => '0755',
            'param_name' => 'files',
    ...
    protected function get_user_path() {
        if ($this->options['user_dirs']) {
            return date('Y'). DS .date('n'). DS . date('j') . DS.$this->get_user_id().DS;
        }
        return '';
    }
    ...
    protected function get_download_url($file_name, $version = null) {
        ...
        if($version=="thumbnail")
            return $this->options['upload_url']."thumb_x80x50/".$this->get_user_path().rawurlencode($file_name);
    ...
    protected function get_file_object($file_name) {
      ...
      //      if(!isset($file->thumnail_url))
                $file->thumbnail_url = $this->options['upload_url']."thumb_x80x50/".$this->get_user_path().rawurlencode($file_name);
    ...
    protected function get_file_objects($iteration_method = 'get_file_object') {
      ...
      //echo $upload_dir;
     //   var_dump(scandir("/www/media.nguoiduatin.vn/public_html/182/2015/4/25"));
    ...
    protected function trim_file_name($name, $type, $index, $content_range) {
        ...
        if (strpos($name, '.') === false &&
            preg_match('/^(gif|png|jpe?g)/', $type, $matches)) {           
            $name .= '.'.$matches[1];
        }        
        return $name;
    }
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error,
        ...
        $image_data = @getimagesize($uploaded_file) ;
   ...
   //     var_dump(count($_name));
        if(count($_name)>1){
           ...
           $name = trim($name,"-");
           $name = $this->templateHelper->build_slug($name);
             $name = $name."-".time().".jpg";     
        }
  ...
  //      echo $name;
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
            ...
            if ($file_size === $file->size) {
                ...
                $file->url = $this->get_download_url($file->name);
                //  if($version=="thumbnail")
                $file->thumbnail_url= $this->options['upload_url']."thumb_x80x50/".$this->get_user_path().rawurlencode($file->name);
                foreach($this->options['image_versions'] as $version => $options) {
