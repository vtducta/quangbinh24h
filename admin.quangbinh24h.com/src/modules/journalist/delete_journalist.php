<?php
    class DeleteJournalist extends Module implements IModule
    {        
        /**
        * permission  model
        * 
        ...
        */
        ...
        public function run()
        {
            ...
            if(!$this->permission_model->check_permission($user_id,array(4)))
            {
                die('no permission');
            }    
            else {
            ...
            $check = $this->journalist_model->get_journalist_by_id($id);
            if (!$check) {
                $this->session->set_session('msg', 'Tac gia khong ton tai');
                Application::redirect($this->linkHelper->link_admin('ListJournalist'));
            }
            else {
                $this->journalist_model->delete_journalist($id);
                $this->session->set_session('msg', 'Xoa thanh cong tac gia id ' . $check['pen_name']);
                Application::redirect($this->linkHelper->link_admin('ListJournalist'));
            }
        }
